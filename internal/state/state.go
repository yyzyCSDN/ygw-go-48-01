...
// RestoreSnapshot replaces committed data with a captured snapshot. Increments
// that arrived after the snapshot was taken are dropped, so keys updated
// concurrently with the restore disappear after the restore.
