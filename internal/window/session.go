...
// ScanTimeout closes sessions whose last activity is older than the timeout
// gap. It evaluates against the confirmed instant supplied by the caller and
// only touches sessions that are still open, keeping the state machine in the
// open -> closed transition exactly once.
func (s *SessionManager) ScanTimeout(now int64) []model.WindowResult {
	...
	for key, ss := range s.sessions {
		...
		if now-ss.lastActive <= s.timeout {
			continue
		}
